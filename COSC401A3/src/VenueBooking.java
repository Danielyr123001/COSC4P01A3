...
//    }
    ...
    public boolean bookVenue(String venueId, LocalDate date) {
        // Check if the venue is available for the given date
        if (checkAvailability(venueId, date)) {
            // If available, book the venue by setting the date to false (not available)
            Map<LocalDate, Boolean> dates = venueAvailability.computeIfAbsent(venueId, k -> new HashMap<>());
            dates.put(date, false); // Set the date to false indicating it's no longer available
            return true;
        }
        return false;
    }
